...
@section('content')
    <form method="post" action="{{ route('login') }}">
        @csrf
        <svg width="130" height="130" viewBox="0 0 250 250"  role="img" aria-labelledby="title  desc">
            <defs>
                <clipPath id="scene">
        ...
        <fieldset id="email-field" class="with-placeholder">
            <legend>Email</legend>
            <div>
                <input value="{{ old('email') }}" type="email" name="email" id="email" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" required  placeholder="Enter your email" />
            </div>
        </fieldset>
        @error('email')
        <span>{{ $message }}</span>
        ...
        <fieldset id="password-field">
            ...
            <div>
                <input type="password" name="password" id="password" autocomplete="nope" autocorrect="off" autocapitalize="off" spellcheck="false" required placeholder="Enter your password"/>
            </div>
        </fieldset>
        @error('password')
        <p style="font-size: 12px;color: red;font-style: italic">{{ $message }}</p>
        @enderror
